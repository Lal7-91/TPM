...
# --------------------------------------------------------------------
def top_flights(flights, preferences, top_n=1):
    try:
...
def run_flights(user_text: str):
    ...
    topFlights_fulldeals = []
    for topFlight in topFlights:
        for fullFlight in flightsFullData.get("best_flights", []) + flightsFullData.get("other_flights", []):
            if topFlight.get("price") == fullFlight.get("price"):
                topFlights_fulldeals.append(fullFlight)
                break

    print("\n✅ Successfully selected top flights.")
    return topFlights_fulldeals
